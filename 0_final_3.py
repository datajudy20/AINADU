...
# train data - front face
data_path = 'f_images/front/'
file_front = [f for f in listdir(data_path) if isfile(join(data_path,f))]

Training_Data, Labels = [], []

for i, files in enumerate(file_front):
    image_path = data_path + file_front[i]
...
# train data - left face
data_path = 'f_images/left/'
file_left = [f for f in listdir(data_path) if isfile(join(data_path,f))]

Training_Data, Labels = [], []

for i, files in enumerate(file_left):
    image_path = data_path + file_left[i]
...
# train data - right face
data_path = 'f_images/right/'
file_right = [f for f in listdir(data_path) if isfile(join(data_path,f))]

Training_Data, Labels = [], []

for i, files in enumerate(file_right):
    image_path = data_path + file_right[i]
...
define_img_size(input_img_size)  # before import module 'create_mb_tiny_fd' and 'create_mb_tiny_fd_predictor'
...
out = cv2.VideoWriter('self.mp4', fourcc, 7, (int(width), int(height)))   # result name : self.mp4
    ...
    for i in range(boxes.size(0)):
     ...
     #   label = f" {probs[i]:.2f}"
        ...
        for s in range(3):
            ...
            print(result[1])      #Smaller values means more similar
